...
async currentOverflowOf ({rootState}, _terminal,_projectId){
				    ...
				    const AccountId = await Accounts.accountAddress();
                     let data = await state.contract.query.currentOverflowOf(AccountId, {value, queryGasLimit},_terminal,_projectId)
...
async currentTotalOverflowOf ({rootState}, _projectId,_decimals,_currency){
				    await judgeContract(rootState.app.web3)
				    const AccountId = await Accounts.accountAddress();
                     let data = await state.contract.query.currentTotalOverflowOf(AccountId, {value, queryGasLimit},_projectId,_decimals,_currency)
                    data = formatResult(data);
                    return data
