...
async function judgeContract(web3,address) {
    if (!state.contract) {
        state.contract = await connectContract(web3, 'MBToken',address)
    }
}
...
const actions = {
    ...
    async balanceOf({rootState}, {coinAddr, owner}) {
        await judgeContract(rootState.app.web3,coinAddr)
        const AccountId = await Accounts.accountAddress();
    ...
    async get_user_check_points({rootState}, account, checkpoint) {
        await judgeContract(rootState.app.web3)
        const AccountId = await Accounts.accountAddress();
        let data = await state.contract.query.get_user_check_points(AccountId, {
            value,
            queryGasLimit
        }, account, checkpoint)
        data = formatResult(data);
